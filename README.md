...
当切换设备后，可以使用两种方式恢复你的备份：通过本地备份恢复或通过云备份恢复。

- 通过本地备份恢复
  
  在旧设备的【备份、云备份、云同步】页面点击【立即备份到本机储存空间】以进行一次手动备份，在对应的储存目录中找到生成的 .pwb 档，将其发送到新设备中，并通过纯纯写作打开，即可恢复本地备份。

- 通过云备份恢复
  
  在确保【备份、云备份、云同步】页面中配置完成云备份后，点击【恢复备份】（此选项亦可以在应用主页右上角云图标的弹出菜单中找到），此时将弹出备份包选择页面，然后在此页面中根据书籍、文章数量，备份时间，备份机型来选择想要恢复的备份。
...
请注意选择**正确的时间和体积**的备份，不要选择你当前新安装又触发而生成的空备份。

## 纯纯写作支持自动同步吗？

<!--待更新-->

纯纯写作暂不支持全自动同步，但支持半自动同步与手动同步。

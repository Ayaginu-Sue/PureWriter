...
本手册专为[纯纯写作](https://writer.drakeet.com/)编写，主要用于纯纯写作中特色功能的介绍与日常使用时的问题自检与修复。

本手册正文分为 [纯纯写作 Q&A](#纯纯写作-qa)、[纯纯写作备份与同步常见问题](#纯纯写作备份与同步常见问题)、[纯纯写作桌面版常见问题解答](#纯纯写作桌面版常见问题解答)、[纯纯写作 Markdown 使用指南](#纯纯写作-Markdown-使用指南)、[纯纯写作 Markdown 使用常见问题](#纯纯写作-markdown-使用常见问题) 五个部分，除此之外，正文之后还有一些其他内容。

在使用中，请善用网页内搜索，电脑端可使用 Ctrl+F 打开网页搜索，手机端请在网页菜单中自行寻找网页搜索。

原则上，GitHub 版本将不会再接受改动范围较小的 Pull request，对手册中的内容有疑问可以发起 Issue。

目前本使用手册所对应的纯纯写作版本较旧，其中部分内容可能已发生了变化，之后将增补并修改更多内容。
...
- [纯纯写作 Q\&A](#纯纯写作-qa)
  ...
  - [关于字数统计](#关于字数统计)
  - [关于横屏下的使用](#关于横屏下的使用)
  - [移动端连接实体键盘时可以使用的快捷键](#移动端连接实体键盘时可以使用的快捷键)
  ...
  - [关于 AI](#关于-ai)
    - [关于使用 AI 时的隐私策略](#关于使用-ai-时的隐私策略)
    - [关于 AI 的使用额度](#关于-ai-的使用额度)
    - [为什么 AI 会撤回一些消息？](#为什么-ai-会撤回一些消息)
    - [为什么 AI 不会回复一些消息？](#为什么-ai-不会回复一些消息)
    - [为什么谷歌用户无法购买私人额度？](#为什么谷歌用户无法购买私人额度)
...
- [纯纯写作桌面版常见问题解答](#纯纯写作桌面版常见问题解答)
  - [关于纯纯写作桌面版及测试版使用体验的重要说明](#关于纯纯写作桌面版及测试版使用体验的重要说明)
  - [桌面版在哪里下载？](#桌面版在哪里下载)
  - [纯纯写作桌面版 2.0 常见问题解答](#纯纯写作桌面版-20-常见问题解答)
    - [如何与其他设备进行同步？](#如何与其他设备进行同步)
    - [可以使用 WebDAV 进行同步吗？](#可以使用-webdav-进行同步吗)
    - [如何新建书籍/卷/篇章？](#如何新建书籍卷篇章)
    - [如何为桌面版切换字体？](#如何为桌面版切换字体)
    - [为何打开的标签页在再次启动时被关闭了？](#为何打开的标签页在再次启动时被关闭了)
  - [纯纯写作桌面版 1.0 常见问题解答](#纯纯写作桌面版-10-常见问题解答)
    - [如何连接桌面版？](#如何连接桌面版)
    - [为什么桌面版打不开？](#为什么桌面版打不开)
...
在使用部分快捷键前需要在顶栏的设置项中打开相应功能的开关。

- 大多数常规快捷键：如 Ctrl+ZXCVYA、Shift+↑↓←→、Home、End 等……
- Tab 缩排/接受 AI Copilot 候选内容 
- ESC 暂时取消 AI 候选内容
- Ctrl+Tab 切换第一/二编辑器
- Ctrl/Alt+num 快捷输入项
- Ctrl/Alt+Enter 发送时光机输入框中的文本
...
纯纯写作编辑器中支持了行间距与段间距，因此在纯文本编辑中，很容易会出现行、段间距不等的情况。在这种情况下，应用本身无法在背景中加入符合已输入及未输入内容的下划线或格；同时，即使仅为已输入的文字加入下划线或格，也很容易会出现线格间距上下不等的问题，严重影响观感。

同样，您也可以发现，市面上所有支持在背景中加入线格的，都不支持单独设置段间距，最多可以修改段落间的自动空行数。
...
**如果此条目未能解决该问题，请向纯纯写作官方（邮箱：writer@drakeet.com）写邮件反馈。**
...
纯纯写作支持在应用中使用 AI 辅助写作，用户可以通过使用纯纯写作 AI 服务或接入 OpenAI API Key 来在纯纯写作中使用 AI 相关的功能，其中纯纯写作 AI 服务是纯纯写作向会员用户提供的服务。

目前纯纯写作中关于 AI 的功能分为三种：时光机对话、Copilot（助手）、AI+。

Copilot 功能为即时提供续写建议。在写作过程中，当光标在文末稍作停留，AI 会自动在光标后生成一段续写建议，您可以点击候选文字来接受它，或通过继续输入来忽略和拒绝它。值得注意的是，若您有私人额度，这可能会导致您的额度被快速消耗。

AI+ 为手动续写功能，在编辑器中长按可以唤出文字菜单，打开开关后其中将包括 AI+ 功能。

### 关于使用 AI 时的隐私策略

纯纯写作**不会、从未也无法**通过任何方式读取用户的文章或使用用户的内容训练 AI，并且用户的文章从未储存在纯纯写作的服务器中，它只存在于用户的设备本地与用户所设定的云盘中。

OpenAI 的官方协议中声明，使用 API 请求的内容将不会留存也不会用于训练 AI 模型。

### 关于 AI 的使用额度

为了方便大部分用户的理解，纯纯写作的 AI 使用额度按使用次数计算而非按照 token 计算。

为了让更多用户体验到最新科技的乐趣，目前纯纯写作为所有会员用户提供了免费的共用额度，其产生的费用由纯纯写作承担。在进行一定程度平均分配的条件下，这些额度由所有会员用户共享，每日分时段逐步发放，先到先得，用完即止。

免费额度分为时光机对话的额度和 Copilot 与 AI+ 的额度，二者额度分开计算，总量为每日共 20000~50000 条。

当用户已购买私人额度时，每次使用 AI 相关功能（包括 Copilot 与 AI+）时会优先消耗每日共用额度，每日共用额度耗尽后消耗私人额度。

### 为什么 AI 会撤回一些消息？

因为一些不可抗力，纯纯写作在本地内置了一个敏感词库，在本地检测 AI 输出的内容是否触及到了敏感词。当 AI 的发言触碰到敏感词时，会自动撤回这条消息。

### 为什么 AI 不会回复一些消息？

因为一些不可抗力，纯纯写作在本地内置了一个敏感词库，在本地检测用户发送给 AI 的内容是否触及到了敏感词。当向 AI 发送的内容触碰到敏感词时，会阻止 AI 回复这条消息。

同理，在包含敏感内容的段落后，Copilot 和 AI+ 将无法正常使用。

### 为什么谷歌用户无法购买私人额度？

受限于谷歌的隐私政策，纯纯写作无法获得谷歌付费用户唯一稳定的 ID 信息，无法准确地为某个用户附加额度信息，因此谷歌用户暂时无法购买私人额度。
...
- 通过云备份恢复
  ...
  确保已在新设备的【备份、云备份、云同步】页面中配置与旧设备相同方式且相同账号的云备份后，点击【恢复备份】（此选项亦可以在应用主页右上角云图标的弹出菜单中找到），在弹出页面中根据书籍、文章数量，备份时间，备份机型来选择想要恢复的云端备份。一般选择来自旧设备的最新备份并进行覆盖备份恢复即可。
...
## 关于纯纯写作桌面版及测试版使用体验的重要说明

纯纯写作桌面版第一次测试目前已结束，第一版中只提供了局域网内双向即时同步编辑当前文章的功能，因此请不要对其抱有太高的期望，以免过度失望。如果您打算**仅仅**因为这个桌面版而付费，我强烈建议您重新考虑这件事情。

纯纯写作桌面版 2 测试目前已经开始，目前仅支持通过 OneDrive 和阿里云盘进行同步。全新的纯纯写作桌面版是一个巨大工程量的产品，至少需要五年以上的时间才能完成，如今发布的客户端实际上是一个半成品。目前许多功能和 UI 都还未实现或处于初始态，大量的细节缺乏打磨和调整……

希望您能为一些细节感到欣喜，同时给予我们一些理解和更多等待时间，将不胜感激，不懈努力。

⚠️ 目前的纯纯写作桌面版中可能会有许多不足和问题，请多担待！⚠️

## 桌面版在哪里下载？

纯纯写作桌面版会在[纯纯写作桌面版官网](https://writer.drakeet.com/desktop) 、[纯纯写作桌面版 2 官网](https://writer.drakeet.com/desktop2) 、[Telegram 频道](https://t.me/PureWriter)、纯纯写作公众号及纯纯写作相关QQ 群、微信群中更新，可以通过相关渠道进行下载。
...
在第一次打开纯纯写作桌面版 2 时，界面的左下角会有云盘登录的选项，可以在界面中登录所使用的云盘，目前云备份仅支持阿里云盘、OneDrive 登录。

在登录云盘后，可以使用云端最新的备份恢复至本地。在登录云盘中有云同步选项，打开该选项后即可进行云同步。

**请注意：**当进行云同步时，本地的内容会与云端的内容进行比对，将更新的一版覆盖至另一边。因此在使用云同步时，需要注意桌面端左下角/手机端右下角的同步标记，当在其他设备有更新的内容时，请勿在另一台设备上修改同一文章，以免造成不必要的损失。

### 可以使用 WebDAV 进行同步吗？

目前桌面版仅支持使用 OneDrive 和阿里云盘进行同步。对 WebDAV 同步的支持目前已在日程中，但优先级相对较低，请耐心等待后续更新。

### 如何新建书籍/卷/篇章？

新建书籍时，需要点击左上角书籍名打开书籍列表，在书籍列表中的右上角可以通过加号新建书籍。

新建卷时，目前在需要书籍内已有篇章，使用鼠标右键单击章节，可在弹出菜单中选择在本章节上或下插入新卷。

新建篇章时，当书籍内无文章时，可以使用顶栏书籍名右侧的加号创建新篇章；当有分卷时，可以右键单击卷，在当前卷末尾创建新篇章；当有其他篇章时，可以右键单击篇章来在此篇章上或下插入新篇章。

### 如何为桌面版切换字体？

在应用内右上角的设置菜单中可以切换纯纯写作桌面版的字体和字号，

纯纯写作中目前支持的字体有 Misans Normal，Klee One Regular，LXGW Bright GB Regular，LXGW WenKai GB，LXGW Bright TC Light，LXGW WenKai TC Regular，Astalia，Xiaolai SC，楷体，仿宋，新宋体等。

受限于安装包大小，部分字体需要自行下载安装为系统字体后才可使用。

### 为何打开的标签页在再次启动时被关闭了？

在纯纯写作新桌面版中，为了防止因打开过多文章而导致启动等待时间过长，纯纯写作会在一定情况下关闭部分标签页以减少启动时间。
...
- 请检查你的设备是否在同一网络下。
- 检查你的 IP 地址是否已经正确输入。
- 检查电脑的防火墙是否阻止了桌面端纯纯写作连接公共网络（或专用网络），如果有，请允许纯纯写作连接至网络。
- 检查你所使用的 WIFI 网络是否打开了 AP 隔离，许多公共场所（如学校、企业、餐饮店、酒店等）的 WIFI 网络会打开 AP 隔离，防止用户之间互相连接造成关键资料遗失。此时可以通过手机/电脑开启热点，另一设备连接热点以解决该问题。

...
## 纯纯写作默认字体会更新吗？
纯纯写作内建的字体是[霞鹜文楷轻便版](https://github.com/lxgw/LxgwWenKai-Lite)，当纯纯写作版本更新时，也会同步保持内建字体为最新状态。
...
- 最常见的情况是由修改字体导致的问题，常见的表现有显示的缩进长度不够（一般为半角空格，即两个全角空格的缩进只有一个汉字字符宽度）或显示为错误符号、圆点、方框等。
...
- 如果有标题中有空格，`#` 中的空格应当删去。（在某些规范中应被替换为 `-`）
...
纯纯写作的 Markdown 遵循 CommonMark 的 0.30 版本，参考链接：[CommonMark Spec](https://spec.commonmark.org/0.30/)
...
**⚠️ 纯纯写作的桌面版仍在测试中，目前的使用体验非常初级，只提供了双向即时同步编辑当前文章这个功能，因此请不要对其抱有太高的期望，以免过度失望。如果您打算*仅仅*因为这个桌面版而付费，我强烈建议您重新考虑这件事情。勿谓言之不预。⚠️**

## 桌面版在哪里下载？

纯纯写作桌面版会在[纯纯写作桌面版下载](http://writer.drakeet.com/desktop_zh) 、Telegram 频道、官方 QQ 群、微信群和公众号中更新，可以通过相关渠道进行下载。
...
这个彩蛋的图标如今也应用于纯纯写作在安卓 12 的启动动画中。
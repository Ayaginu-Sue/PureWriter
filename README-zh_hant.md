...
## 如何配置雲備份？
在純純寫作的設置頁面內打開【備份、雲備份、雲同步】頁面。
想了解更多或了解關於堅果雲的備份配置可以訪問[雲備份與自動備份教程](https://writer.drakeet.com/backups)

## 如何在新設備中恢復備份？

當切換設備後，可以使用兩種方式恢復你的備份：通過本地備份恢復或通過雲備份恢復。

- 通過本地備份恢復
  
  在舊設備的【備份、雲備份、雲同步】頁面點擊【立即備份到本機儲存空間】以進行一次手動備份，在對應的儲存目錄中找到生成的 .pwb 檔，將其發送到新設備中，並通過純純寫作打開，即可恢復本地備份。

- 通過雲備份恢復
  
  在確保【備份、雲備份、雲同步】頁面中配置完成雲備份後，點擊【恢復備份】（此選項亦可以在應用主頁右上角雲圖標的彈出菜單中找到），此時將彈出備份包選擇頁面，然後在此頁面中根據書籍、文章數量，備份時間，備份機型來選擇想要恢復的備份。
...
- 盡量保證無線信號的強度和連接穩定性，避免因距離路由器距離過遠而導致連線不穩定。目前可以採用的解決方式是使用手機連接 WLAN（非必選，但此方式能節省流量且不會增加連線頁面的 IP 數量）後開啟熱點，然後使用電腦連接熱點，使二者保持較近距離。
...
可以嘗試修改 Nvidia 控制面板的刷新率設置為固定刷新率以解決該問題。

...
- [純純寫作備份與同步常見問題](#純純寫作備份與同步常見問題)
  ...
  - [如何配置雲備份？](#如何配置雲備份)
  - [如何在新設備中恢復舊設備中的文章？](#如何在新設備中恢復舊設備中的文章)
  ...
  - [純純寫作可以使用百度雲/阿里雲備份嗎？](#純純寫作可以使用百度雲阿里雲備份嗎)
...
## 純純寫作默認字體會更新嗎？
純純寫作內建的字體是[霞鶩文楷輕便版](https://github.com/lxgw/LxgwWenKai-Lite)，當純純寫作版本更新時，也會同步保持內建字體為最新狀態。
...
純純寫作暫時不支持導出其他格式的檔案。有計劃支持導出 .docx 檔案，但該計劃優先級較低；導出 .epub 等格式所涉及的改動太多，可行性留待之後討論。
...
## 如何註銷純純寫作賬號？
如果您需要註銷您的純純寫作賬號，請通過您的註冊郵箱發送電子郵件至我們的郵箱drakeet@drakeet.com，並寫明您要註銷的賬號名，我們會很快幫您完成註銷並回信告知您。
...
WebDAV 雲備份相當簡單，你可以訪問[《純純寫作-自動備份與雲備份教程》](https://writer.drakeet.com/backups)來了解純純寫作的自動備份和雲備份功能，並獲得註冊堅果雲的教程。

雲備份賬號與純純寫作的賬號不是綁定的，純純寫作的賬號僅與激活狀態綁定。恢復備份即可恢復純純寫作的激活狀態，但登錄純純寫作賬號無法恢復文章備份。

如果用戶不設置雲備份，則應手動將本地備份保存在可以信任的位置，以便不時之需。
...
當切換設備後，可以使用兩種方式恢復你的備份：通過雲備份恢復或通過本地備份恢復。

- 通過雲備份恢復
  
  在確保新設備的【備份、雲備份、雲同步】頁面中已配置完成雲備份後，點擊【恢復備份】（此選項亦可以在應用主頁右上角雲圖標的彈出菜單中找到），在彈出頁面中根據書籍、文章數量，備份時間，備份機型來選擇想要恢復的備份。一般選擇來自舊設備的最新備份並進行覆蓋備份恢復即可。

- 通過本地備份恢復
  
  沒有進行雲備份，並拒絕純純寫作雲備份提醒的用戶，請經常手動保存本地備份至可信任的位置。
...
請注意選擇**來自舊設備的時間和體積正確**的備份，不要選擇你當前新安裝而觸發生成的空備份。
...
- 使用雲同步時
  ...
  根據手動同步和半自動同步選擇的不同，文章將在不同設備間進行雲同步。手動同步需要手動點擊位於編輯器底部的同步按鈕以進行同步；半自動同步則會在文字有變更且收起鍵盤時自動進行同步，兩次同步的間隔至少為 6 秒。
...
百度雲目前已不再公開提供 API 服務，只對部分廠商特別提供，阿里雲目前未提供任何公開的 API 供第三方應用接入，使用非公開的 API 可能構成侵權行為，因此純純寫作無法提供接入百度與阿里的雲服務的功能。
...
由於眾所周知的原因，中國大陸部分用戶使用 OneDrive 雲服務的速度並不理想，因此純純寫作在無法保證不會因此造成文章丟失的情況下，將不會支持使用 OneDrive 備份。

微雲、百度網盤、阿里雲盤、微盤等國內網盤通常是不開放的網盤，並不提供（或僅對部分廠商提供）第三方應用可使用的 API 接口，並且可能還存在許多其他影響使用的因素（如限速等），因此純純寫作暫不會接入國內的其他網盤。
...
這個彩蛋的圖標如今也應用於純純寫作在安卓 12 的啟動動畫中。
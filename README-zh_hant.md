...
本手冊專為[純純寫作](https://writer.drakeet.com/)編寫，主要用於純純寫作中特色功能的介紹與日常使用時的問題自檢與修復。

本手冊正文分為 [純純寫作 Q&A](#純純寫作-qa)、[純純寫作備份與同步常見問題](#純純寫作備份與同步常見問題)、[純純寫作桌面版常見問題解答](#純純寫作桌面版常見問題解答)、[純純寫作 Markdown 使用指南](#純純寫作-Markdown-使用指南)、[純純寫作 Markdown 使用常見問題](#純純寫作-markdown-使用常見問題) 五個部分，除此之外，正文之後還有一些其他內容。

在使用中，請善用網頁內搜索，電腦端可使用 Ctrl+F 打開網頁搜索，手機端請在網頁菜單中自行尋找網頁搜索。

原則上，GitHub 版本將不會再接受改動範圍較小的 Pull request，對手冊中的內容有疑問可以發起 Issue。

目前本使用手冊所對應的純純寫作版本較舊，其中部分內容可能已發生了變化，之後將增補並修改更多內容。
...
- [純純寫作 Q\&A](#純純寫作-qa)
  ...
  - [關於字數統計](#關於字數統計)
  - [關於橫屏下的使用](#關於橫屏下的使用)
  - [移動端連接實體鍵盤時可以使用的快捷鍵](#移動端連接實體鍵盤時可以使用的快捷鍵)
...
- [純純寫作桌面版常見問題解答](#純純寫作桌面版常見問題解答)
  - [關於純純寫作桌面版及測試版使用體驗的重要說明](#關於純純寫作桌面版及測試版使用體驗的重要說明)
  - [桌面版在哪裡下載？](#桌面版在哪裡下載)
  - [純純寫作桌面版 2.0 常見問題解答](#純純寫作桌面版-20-常見問題解答)
    - [如何與其他設備進行同步？](#如何與其他設備進行同步)
    - [可以使用 WebDAV 進行同步嗎？](#可以使用-webdav-進行同步嗎)
    - [如何新建書籍/卷/篇章？](#如何新建書籍卷篇章)
    - [如何為桌面版切換字體？](#如何為桌面版切換字體)
    - [為何打開的標籤頁在再次啟動時被關閉了？](#為何打開的標籤頁在再次啟動時被關閉了)
  - [純純寫作桌面版 1.0 常見問題解答](#純純寫作桌面版-10-常見問題解答)
    - [如何連接桌面版？](#如何連接桌面版)
    - [為什麼桌面版打不開？](#為什麼桌面版打不開)
...
在使用部分快捷鍵前需要在頂欄的設置項中打開相應功能的開關。

- 大多數常規快捷鍵：如 Ctrl+ZXCVYA、Shift+↑↓←→、Home、End 等……
- Tab 縮排/接受 AI Copilot 候選內容 
- ESC 暫時取消 AI 候選內容
- Ctrl+Tab 切換第一/二編輯器
- Ctrl/Alt+num 快捷輸入項
- Ctrl/Alt+Enter 發送時光機輸入框中的文本
...
**如果此條目未能解決該問題，請向純純寫作官方（郵箱：writer@drakeet.com）寫郵件反饋。**
...
如果您需要註銷您的純純寫作賬號，請進入純純寫作 - 設置 - 少數派選項 - 註銷純純寫作賬號，「註銷純純寫作賬號」在「少數派選項」頁面底部，點擊即可自助進行註銷純純寫作賬號。

**注意：** 如果您沒有純純寫作 Pro 會員，那麼進入「少數派選項」會見到「這個頁面需要高級版」提示，您可以點擊對話框中的「試用」按鈕以忽略它，然後即可滾動頁面到底部找到註銷選項，**註銷完全不必要純純寫作 Pro 會員，請放心！**

## 關於 AI

純純寫作支持在應用中使用 AI 輔助寫作，用戶可以通過使用純純寫作 AI 服務或接入 OpenAI API Key 來在純純寫作中使用 AI 相關的功能，其中純純寫作 AI 服務是純純寫作向會員用戶提供的服務。

目前純純寫作中關於 AI 的功能分為三種：時光機對話、Copilot（助手）、AI+。
...
目前純純寫作支持使用 WebDAV、Dropbox、Google Drive、OneDrive 和 阿里雲盤 同步。
...
## 關於純純寫作桌面版及測試版使用體驗的重要說明

純純寫作桌面版第一次測試目前已結束，第一版中只提供了局域網內雙向即時同步編輯當前文章的功能，因此請不要對其抱有太高的期望，以免過度失望。如果您打算**僅僅**因為這個桌面版而付費，我強烈建議您重新考慮這件事情。

純純寫作桌面版 2 測試目前已經開始，目前僅支持通過 OneDrive 和阿里雲盤進行同步。全新的純純寫作桌面版是一個巨大工程量的產品，至少需要五年以上的時間才能完成，如今發布的客戶端實際上是一個半成品。目前許多功能和 UI 都還未實現或處於初始態，大量的細節缺乏打磨和調整……

希望您能為一些細節感到欣喜，同時給予我們一些理解和更多等待時間，將不勝感激，不懈努力。

⚠️ 目前的純純寫作桌面版中可能會有許多不足和問題，請多擔待！⚠️

## 桌面版在哪裡下載？

純純寫作桌面版會在[純純寫作桌面版官網](https://writer.drakeet.com/desktop) 、[純純寫作桌面版 2 官網](https://writer.drakeet.com/desktop2) 、[Telegram 頻道](https://t.me/PureWriter)、純純寫作公眾號及純純寫作相關QQ 群、微信群中更新，可以通過相關渠道進行下載。
...
在第一次打開純純寫作桌面版 2 時，界面的左下角會有雲盤登錄的選項，可以在界面中登錄所使用的雲盤，目前雲備份僅支持阿里雲盤、OneDrive 登錄。

在登錄雲盤後，可以使用雲端最新的備份恢復至本地。在登錄雲盤中有雲同步選項，打開該選項後即可進行雲同步。

**請注意：**當進行雲同步時，本地的內容會與雲端的內容進行比對，將更新的一版覆蓋至另一邊。因此在使用雲同步時，需要注意桌面端左下角/手機端右下角的同步標記，當在其他設備有更新的內容時，請勿在另一台設備上修改同一文章，以免造成不必要的損失。

### 可以使用 WebDAV 進行同步嗎？

目前桌面版僅支持使用 OneDrive 和阿里雲盤進行同步。對 WebDAV 同步的支持目前已在日程中，但優先級相對較低，請耐心等待後續更新。

### 如何新建書籍/卷/篇章？

新建書籍時，需要點擊左上角書籍名打開書籍列表，在書籍列表中的右上角可以通過加號新建書籍。

新建卷時，目前在需要書籍內已有篇章，使用鼠標右鍵單擊章節，可在彈出菜單中選擇在本章節上或下插入新卷。

新建篇章時，當書籍內無文章時，可以使用頂欄書籍名右側的加號創建新篇章；當有分卷時，可以右鍵單擊卷，在當前卷末尾創建新篇章；當有其他篇章時，可以右鍵單擊篇章來在此篇章上或下插入新篇章。

### 如何為桌面版切換字體？

在應用內右上角的設置菜單中可以切換純純寫作桌面版的字體和字號，

純純寫作中目前支持的字體有 Misans Normal，Klee One Regular，LXGW Bright GB Regular，LXGW WenKai GB，LXGW Bright TC Light，LXGW WenKai TC Regular，Astalia，Xiaolai SC，楷體，仿宋，新宋體等。

受限於安裝包大小，部分字體需要自行下載安裝為系統字體後才可使用。

### 為何打開的標籤頁在再次啟動時被關閉了？

在純純寫作新桌面版中，為了防止因打開過多文章而導致啟動等待時間過長，純純寫作會在一定情況下關閉部分標籤頁以減少啟動時間。
...
- 在純純寫作中，使用默認的霞鶩文楷字體，複製或輸入 （U+E003）即可見到純純寫作的時光機圖標。
  
  該字符在當前頁面中可能無法正常顯示，將它複製到純純寫作中後套用霞鶩文楷即可正常顯示。

- 在某些情況下，也許可以看到更多的 Shoutcut。
...
如果你無法登錄 Google 服務，則無法激活純純寫作，所有你在 Google Play 上付費的應用都是這樣的。並不是純純寫作才這樣。因此您應該向該設備反饋，讓其保留谷歌服務，至少可自行安裝。
...
在純純寫作的時光機中存在一些隱藏的魔法代碼，在時光機中輸入魔法代碼的話，可能會產生一些有趣的效果。

## 純純寫作的小彩蛋

目前已證實存在的彩蛋有兩個。

- 在會員頁面進行一定次數的某些重複操作後，可以見到純純寫作曾經設計過但最終放棄的一個圖標和一個彈窗彩蛋。
  
  這個彩蛋的圖標如今也被應用於純純寫作在安卓 12 的啟動動畫中。
  
- 在純純寫作中，使用默認的霞鶩文楷字體，複製或輸入 （U+E003）即可見到這個彩蛋。
  
  該字符在當前頁面中可能無法正常顯示，將它複製到純純寫作中後套用霞鶩文楷即可正常顯示。